...
### 伪装网站的选择
自建VPS使用Xray进行代理的流量特征有**单点性**和**大流量性**。**单点性**指使用的人少，一般只有自己，即时分享给朋友，一般也不会太多。

最符合这两个特征的伪装网站就是个人网盘。个人网盘可以理解为使用自己的VPS搭建起来的百度网盘，区别就是文件都存放在VPS中，并且自己是网盘的管理员。

网盘可以自己用，也可以分享给几个人用，甚至可以商用，无论用户多少都合理，符合**单点性**和**大流量性**的特征。
