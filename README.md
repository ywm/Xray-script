...
### 伪装网站的选择
自建VPS使用Xray进行代理的流量特征有 **单点性** 、 **大流量性** 和 **出入相同性** 。**单点性** 指使用的人少，一般只有自己，即使分享给朋友，一般也不会太多。**出入相同性** 指入VPS和出VPS的流量在时间和大小上几乎相同，比如使用Xray代理浏览`BiliBili`，从`BiliBili`到`VPS(Xray服务端)`的流量，和从`VPS`到`Xray客户端`的流量在时间上和大小上是几乎相同的。**出入相同性** 是所有代理的通病，目前还没有太好的伪装方法。

目前最符合这几个特征的伪装网站就是个人网盘。个人网盘可以理解为使用自己的VPS搭建起来的百度网盘，区别就是文件都存放在VPS中，并且自己是网盘的管理员。

网盘可以自己用，也可以分享给几个人用，甚至可以商用，无论用户多少都合理，符合 **单点性** 和 **大流量性** 。

许多脚本使用博客网站伪装，实际上博客网站往往提供给很多人浏览，并且流量不是很大，与 **单点性** 和 **大流量性** 相悖。
1. **Cloudreve 和 Nextcloud**

他们都是个人网盘，伪装性最强，建议选择。他们的区别如下：
||优点|缺点|
|-|-|-|
