...
cleanup_temp() {
    ...
    # 清理根目录可能的残留临时文件
    rm -f /temp.c /temp.cpp 2>/dev/null || true
}
# 用户中断信号处理
handle_interrupt()
{
...
check_script_update()
{
    [ "$(md5sum "${BASH_SOURCE[0]}" | awk '{print $1}')" == "$(md5sum <(wget -O - "https://github.com/ywm/Xray-script/raw/main/Xray-TLS+Web-setup.sh") | awk '{print $1}')" ] && return 1 || return 0
}
#更新脚本
update_script()
{
    if wget -O "${BASH_SOURCE[0]}" "https://github.com/ywm/Xray-script/raw/main/Xray-TLS+Web-setup.sh" || wget -O "${BASH_SOURCE[0]}" "https://github.com/ywm/Xray-script/raw/main/Xray-TLS+Web-setup.sh"; then
    ...
    if [ -d "$xray_config_dir" ]; then
        ...
        if [ -f "$xray_config_inbound_xhttp" ]; then
            ...
            if [ -f "/usr/local/etc/xray/vlessenc_keys.json" ]; then
                xhttp_encryption="$(jq -r '.encryption' /usr/local/etc/xray/vlessenc_keys.json 2>/dev/null || echo '')"
                xhttp_decryption="$(jq -r '.decryption' /usr/local/etc/xray/vlessenc_keys.json 2>/dev/null || echo '')"
            fi
            # 如果密钥丢失或解析失败（为空），且 xray 二进制存在，尝试生成
            if { [ -z "$xhttp_encryption" ] || [ -z "$xhttp_decryption" ]; } && [ -f "/usr/local/bin/xray" ]; then
                local temp_keys
                if temp_keys=$(/usr/local/bin/xray vlessenc 2>/dev/null) && echo "$temp_keys" | grep -q '"encryption"'; then
                    echo "$temp_keys" > /usr/local/etc/xray/vlessenc_keys.json
                    xhttp_encryption="$(echo "$temp_keys" | jq -r '.encryption' 2>/dev/null)"
                    xhttp_decryption="$(echo "$temp_keys" | jq -r '.decryption' 2>/dev/null)"
                fi
            fi
        ...
        if (( 1<=choice&&choice<=4 )); then
            if (( choice==1 || choice==4 )) && ([[ "$release" == "ubuntu" ]] || [[ "$release" == "debian" ]] || [ $release == "deepin" ] || [[ "$release" == "other-debian" ]]) && ! dpkg-deb --help | grep -qw "zstd"; then
                ...
                if (( choice==1 || choice==4 )) && ([[ "$release" == "ubuntu" ]] || [[ "$release" == "debian" ]] || [ $release == "deepin" ] || [[ "$release" == "other-debian" ]]) && ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                    ...
                    if [[ $choice -eq 1 ]]; then
                        temp_kernel_sh_url="https://github.com/ywm/update-kernel/raw/master/update-kernel-stable.sh"
                    elif [[ $choice -eq 4 ]]; then
                        temp_kernel_sh_url="https://github.com/ywm/update-kernel/raw/master/update-kernel.sh"
                    else
                        temp_kernel_sh_url="https://github.com/ywm/xanmod-install/raw/main/xanmod-install.sh"
                    fi
                    if ! wget -O xanmod-install.sh "$temp_kernel_sh_url"; then
                        red    "获取内核安装脚本失败"
...
readPretend()
{
    ...
    do
        ...
        echo
        yellow " 关于选择伪装网站的详细说明见：https://github.com/ywm/Xray-script#伪装网站说明"
        echo
        pretend=""
    ...
    "inbounds": [
        ...
        },
        {
            ...
            "settings": {
                ...
                "decryption": "${xhttp_decryption:-none}"
            },
            "streamSettings": {
                "network": "xhttp",
                "xhttpSettings": {
...
install_update_xray_tls_web()
{
    ...
    if [ $use_existed_nginx -eq 0 ]; then
        ...
        if [ -d "${nginx_prefix}/certs" ]; then
            ...
            cp -r "${nginx_prefix}/certs/"* "${temp_dir}/certs_backup/" 2>/dev/null
        fi
    ...
    if [ $use_existed_nginx -eq 0 ] && [ -d "${temp_dir}/certs_backup" ]; then
        green "恢复备份的证书..."
        mkdir -p "${nginx_prefix}/certs"
        cp -r "${temp_dir}/certs_backup/"* "${nginx_prefix}/certs/" 2>/dev/null
    fi
...
proxies:
  ...
  - name: "VLESS-XHTTP-TLS"
    ...
    alpn:
      - h2
    network: xhttp
    encryption: "${xhttp_encryption:-none}"
    xhttp-opts:
      path: "${path}"
      host: "${xhttp_domain}"
      mode: "${xhttp_mode}"
